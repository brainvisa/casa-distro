...
#RSYNC_VERSION=3.1.2
RSYNC_VERSION=3.0.8
RSYNC_INSTALL_PREFIX=${CROSSBUILD_INSTALL_PREFIX}/rsync-${RSYNC_VERSION}
RSYNC_INSTALL_PREFIX_WINE=$(winepath -w ${RSYNC_INSTALL_PREFIX})
RSYNC_SOURCE_URL=https://download.samba.org/pub/rsync/src/rsync-${RSYNC_VERSION}.tar.gz
if [ "${RSYNC}" == "1" ]; then
    echo "============================== RSYNC ==============================="
    if [ "${__download}" == "1" ]; then
        download ${RSYNC_SOURCE_URL}
    fi

    if [ "${__remove_before_install}" == "1"  ]; then
        remove ${RSYNC_INSTALL_PREFIX}
    fi

    if [ "${__install}" == "1" ]; then
        tar xvf ${__download_dir}/rsync-${RSYNC_VERSION}.tar.gz
        pushd ${__build_dir}/rsync-${RSYNC_VERSION}

        cat << EOF > rsync-${RSYNC_VERSION}.patch
diff -urN configure.ac configure.ac
--- configure.ac	2011-03-26 17:34:18.000000000 -0400
+++ configure.ac	2011-08-23 23:03:52.519000000 -0400
@@ -196,7 +196,7 @@
...
+	*cygwin* | *msys* | *mingw*) AC_MSG_RESULT(yes)
...
+dnl cygwin/msys iconv.h defines iconv_open as libiconv_open
...
@@ -883,7 +883,7 @@
 	...
 	;;
-    *solaris*|*cygwin*)
+    *solaris*|*cygwin*|*msys*|*mingw*)
 	AC_MSG_RESULT(Using solaris ACLs)
 	AC_DEFINE(HAVE_SOLARIS_ACLS, 1, [true if you have solaris ACLs])
 	AC_DEFINE(SUPPORT_ACLS, 1)
diff -urN lib/getaddrinfo.c lib/getaddrinfo.c
--- lib/getaddrinfo.c	2009-12-12 20:20:35.000000000 -0500
+++ lib/getaddrinfo.c	2011-08-23 23:05:06.823000000 -0400
@@ -60,7 +60,11 @@
...
+# endif
 	...
 	if (!hp) {
 		switch (h_errno) {
diff -urN lib/sysacls.h lib/sysacls.h
--- lib/sysacls.h	2008-03-01 15:01:41.000000000 -0500
+++ lib/sysacls.h	2011-08-23 23:05:30.498000000 -0400
@@ -131,7 +131,7 @@
...
diff -urN util.c util.c
--- util.c	2011-03-17 14:25:12.000000000 -0400
+++ util.c	2011-08-23 23:06:53.549000000 -0400
@@ -834,7 +834,7 @@
...
diff -urN zlib/zutil.h zlib/zutil.h
--- zlib/zutil.h	2005-07-22 11:54:43.000000000 -0400
+++ zlib/zutil.h	2011-08-23 23:08:01.348000000 -0400
@@ -141,7 +141,8 @@
 #endif
...
--- Makefile.in	2011-03-26 13:01:37.000000000 -0400
+++ Makefile.in	2011-08-24 00:43:27.168000000 -0400
@@ -66,8 +66,8 @@
...
diff -urN rsync.h rsync.h
--- rsync.h	2011-02-21 14:32:51.000000000 -0500
+++ rsync.h	2011-08-24 00:52:56.686000000 -0400
@@ -466,6 +466,14 @@
 #define schar char
 #endif
...
diff -NurB --strip-trailing-cr --suppress-common-lines uidlist.c uidlist.c
--- uidlist.c	2018-01-15 12:44:45.186273917 +0100
+++ uidlist.c	2018-01-15 12:45:23.278118369 +0100
@@ -24,7 +24,7 @@
  * are special. */
...
+#include "rsync_io.h"
 ...
 extern int verbose;
 extern int am_root;
diff -NurB --strip-trailing-cr --suppress-common-lines main.c main.c
--- main.c	2018-01-15 12:44:36.906307743 +0100
+++ main.c	2018-01-15 12:45:18.530137752 +0100
@@ -22,7 +22,7 @@
...
+#include "rsync_io.h"
 ...
 #include <locale.h>
 #endif
diff -NurB --strip-trailing-cr --suppress-common-lines io.h io.h
--- io.h	2018-01-15 12:43:49.050503323 +0100
+++ io.h	1970-01-01 01:00:00.000000000 +0100
@@ -1,52 +0,0 @@
-/*
...
-}
diff -NurB --strip-trailing-cr --suppress-common-lines flist.c flist.c
--- flist.c	2018-01-15 12:44:52.650243432 +0100
+++ flist.c	2018-01-15 12:45:11.786165274 +0100
@@ -23,7 +23,7 @@
 #include "rsync.h"
 #include "ifuncs.h"
 #include "rounding.h"
...
+#include "rsync_io.h"
 ...
 extern int verbose;
 extern int am_root;
diff -NurB --strip-trailing-cr --suppress-common-lines aclocal.m4 aclocal.m4
--- aclocal.m4	2018-01-15 12:38:05.955909994 +0100
+++ aclocal.m4	2018-01-15 12:40:05.427419250 +0100
@@ -33,19 +33,19 @@
...
diff -NurB --strip-trailing-cr --suppress-common-lines rsync_io.h rsync_io.h
--- rsync_io.h	2018-01-15 12:43:58.978462729 +0100
+++ rsync_io.h	2018-01-15 12:43:41.042536074 +0100
@@ -0,0 +1,52 @@
+/*
+ * Copyright (C) 2007-2008 Wayne Davison
+ *
...
EOF
         patch -f -N -i rsync-${RSYNC_VERSION}.patch -p0
         #rm -f io.h
         ...
         && ./configure \
                --build=${__buildtype} \
                --host=${__toolchain} \
                --prefix=${RSYNC_INSTALL_PREFIX} \
        || exit 1
        make -j${__build_proc_num} install || exit 1

        pushd ${CROSSBUILD_INSTALL_PREFIX}
        rm -f rsync && ln -fs rsync-${RSYNC_VERSION} rsync
        pushd bin;ln -fs ../rsync/bin/*.* ./;popd
